...
local TemporalLogExPoolingGC, parent = torch.class('nn.TemporalLogExPoolingGC', 'nn.Module')

function TemporalLogExPoolingGC:__init(kW, dW, beta,delta)
   parent.__init(self)
...
function TemporalLogExPoolingGC:updateOutput(input) -- MODIFY THE NAME BACK TO TemporalLogExpPooling
   -----------------------------------------------
...
function TemporalLogExPoolingGC:updateGradInput(input, gradOutput)

   -----------------------------------------------
   --print("GC delta =",self.delta)
   if input:dim() ==  2 then --2D
      ...
      for i=1,input:size(2) do
         ...
         for j= 1,gradOutput:size(1) do--for each value in gradOutput[j,{}], we should distribute the value to grdInput
            ...
            for k=(j-1)*self.dW + 1,(j-1)*self.dW + self.kW do
               --gradInput[{k,i}] = gradInput[{k,i}] + z * math.exp(self.beta * input[{k,i}]) / sum_of_exp_xk
               dE_dXk = 1/self.beta * math.log(( sum_of_exp_xk-math.exp(self.beta * input[{k,i}]) + math.exp(self.beta * (input[{k,i}] + self.delta) ) )/self.kW )
               dE_dXk = dE_dXk - 1/self.beta * math.log(sum_of_exp_xk / self.kW)
               dE_dXk = dE_dXk /self.delta 

               gradInput[{k,i}] = gradInput[{k,i}] + z * dE_dXk;
               --gradInput[{k,i}] = gradInput[{k,i}] + z * math.exp(self.beta * input[{k,i}]) / ( sum_of_exp_xk )
            end
         ...
         for i=1,input:size(3) do
            ...
            for j= 1,gradOutput:size(2) do--for each value in gradOutput[j,{}], we should distribute the value to grdInput
               ...
               for k=(j-1)*self.dW + 1,(j-1)*self.dW + self.kW do
                  ...
                  dE_dXk = 1/self.beta * math.log(( sum_of_exp_xk-math.exp(self.beta * input[{h,k,i}]) + math.exp(self.beta * (input[{h,k,i}] + self.delta)  ) )/self.kW  )
                  dE_dXk = dE_dXk - 1/self.beta * math.log(sum_of_exp_xk / self.kW)
                  dE_dXk = dE_dXk /self.delta 

                  gradInput[{h,k,i}] = gradInput[{h,k,i}] + z * dE_dXk;
                  --gradInput[{h,k,i}] = gradInput[{h,k,i}] + z * math.exp(self.beta * input[{h,k,i}]) / sum_of_exp_xk
               end
...
function TemporalLogExPoolingGC:empty()
   self.gradInput:resize()
   self.gradInput:storage():resize(0)
   self.output:resize()
   self.output:storage():resize(0)
...
function main()
   ...
   delta = 0.01
   model = nn.Sequential()
   model:add(nn.TemporalLogExPooling(3,1,1))

   model2 = nn.Sequential()
   model2:add(nn.TemporalLogExPoolingGC(3,1,1,delta))
  ...
  x = torch.Tensor(3,4,5)
   ...
   ouGC = model2:forward(x)
   ou2GC = model2:backward(x,ouGC)

   diff = torch.sum(torch.pow(ou2GC - ou2 ,2))
   print("Squre Error between infinite difference finite differencce is",diff)
end
